import './Tile.css';
import { useDaily, useLocalSessionId, DailyVideo, useVideoTrack, useParticipantProperty, useParticipant } from '@daily-co/daily-react';
...
export default function Tile(props) {
  ...
  const userLifeData = [];
  const [lifeTotal, lifeLog, userName] = useParticipantProperty(id, ['userData.lifeTotal', 'userData.lifeLog', 'user_name']);
  const [lifeLogTableArray, setLifeLogTableArray] = useState();
  const participantState = useParticipant(id);
    ...
    if(lifeLog) {
      ...
      lifeLog.forEach( log => {
        ...
        if(existingData !== undefined){
          ...
          lifeTableData[log.damageSource].damageTaken = lifeTableData[log.damageSource].damageTaken + damageTakenValue;
        } else {
